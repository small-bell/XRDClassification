...
class Model():
    ...
    def __init__(self, level=0, num_works=1, batch_size=8, epoch=500, train_mod=False):
        if train_mod:
            self.dataset = CellMemoryDataset(level=level)
        else:
            self.dataset = CellDiskDataset(level=level)
        self.num_classes = self.dataset.types_num
        net = SEResNet(num_classes=self.dataset.types_num)
        print(net.eval())

        self.root_path = './{}/model_results/'.format(level)
        ...
        self.weight = torch.tensor(self.cal_weight(), dtype=torch.float).to(self.device)
        self.loss_fn = WeightedMultilabel(weights=self.weight)

    def cal_weight(self):
        return 10000. / np.array(self.dataset.single_types_num)
...
if __name__ == '__main__':
    for i in range(10):
        ...
        model = Model(level=i, num_works=13, batch_size=128, epoch=30, train_mod=True)
        model.fit()
        if i == 1:
            break
